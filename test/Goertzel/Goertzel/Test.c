...
static void generate_sinusoids(int fs, int fo, int size, U16 * sinusoid)
{
	...
	for(;i < size ; ++i)
	{
 		sinusoid[i] = (U16)(A * sin((2*PI*fo*i) / (float)fs)) + (U16)(A * sin((2*PI*123*i) / (float)fs)) + (U16)(A * sin((2*PI*125*i) / (float)fs));
	}
	/*
	for(; i<size;++i)
	{
...
static void generate_sinusoids_double_samples(int fs, int fo, int size, double* sinusoid)
{
	...
	for(;i < size ; ++i)
	{
 		sinusoid[i] = (double)(A * sin((2*PI*fo*i) / (double)fs)) + (double)(A * sin((2*PI*880*i) / (float)fs)) + (double)(A * sin((2*PI*110*i) / (float)fs)) + (double)(A * sin((2*PI*55*i) / (float)fs))+ (double)(A * sin((2*PI*1760*i) / (float)fs))+ (double)(A * sin((2*PI*3520*i) / (float)fs));
	}
