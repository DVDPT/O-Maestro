...
class UScheduler
{
	...
	///
	SECTION(".internalmem") static IsrCompletationStatus SystemTimerInterruptRoutine(InterruptArgs* args, SystemIsrArgs sargs);
	...
	///
	SECTION(".internalmem") static void SystemTimerPostInterruptRoutine(SystemPisrArgs args);

	///
	///	Removes the thread passed as argument from its ready list
	///
	SECTION(".internalmem") static void RemoveThreadFromReadyQueue(UThread& thread);

	///
	///	Inserts this thread passed as argument in its ready queue
	///
	SECTION(".internalmem") static void InsertThreadInReadyQueue(UThread& thread);
	...
	///
	SECTION(".internalmem") static UThread& DequeueNextReadyThread();

	///
	///	Returns the next ready thread.
	///		NOTE: this function always return a thread, because idle thread never blocks
	///
	SECTION(".internalmem") static UThread* PeekNextReadyThread();

	///
	///	Returns TRUE when there is a ready thread with a bigger priority than the running thread. Returns FALSE otherwise.
	///
	SECTION(".internalmem") static bool HaveReadyThreads();
	...
	///
	SECTION(".internalmem") static UThread& Schedule();
	///
	...
	///
	SECTION(".internalmem") static void UnlockInner(U32 lockCount);
	...
	///		for porting reasons
	///
	SECTION(".internalmem") PORT_SPECIFIC NAKED static void ContextSwitch(UThread * old, UThread * next);
