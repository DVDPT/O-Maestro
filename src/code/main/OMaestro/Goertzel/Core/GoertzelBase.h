...
#define GOERTZEL_FILTERED_SAMPLES_POWER_MINIMUM_PERCENTAGE_THRESHOLD (7)
...
SECTION(".internalmem") static GoertzelInternalPowerType CalculateRelativePower(GoertzelInternalPowerType Q1, GoertzelInternalPowerType Q2, double coeff)
	{
		return (Q1*Q1) + (Q2*Q2) - Q1 * Q2 * coeff;		
	}

SECTION(".internalmem") static GoertzelInternalPowerType CalculateFrequencyPower(GoertzelInternalPowerType relativePower, int n)
	{
...
public:
	...
	NOINLINE SECTION(".internalmem") static bool CalculateGoertzel(GOERTZEL_CONTROLLER_SAMPLES_TYPE * samples, int samplesSize,GoertzelFrequency * freq , GoertzelResult * result,const GoertzelPowerType totalPower);
};
